...
**Containerized worker pool — Plan A built (A4 slice 1).** The agent worker now runs as a role-configured, scalable Docker service (`Dockerfile` + `worker` compose service): a `naaf_worker_roles` setting scopes each worker, and `claim_next(roles)` makes it claim only the bus messages for its role(s). Roles are **partitioned one-per-worker** (documented invariant) to preserve one-in-flight-per-recipient; the per-recipient advisory-lock hardening for multiple-workers-per-role is a follow-up. **Plan B** (real `LocalWorkspace` adapter + `Scm`/GitHub App + `PROVISION`/`PR` wiring → real PRs) and **egress hardening** (slice 3) come next. See [superpowers/plans/2026-07-02-worker-pool-plan-a.md](superpowers/plans/2026-07-02-worker-pool-plan-a.md) (design: [superpowers/specs/2026-07-02-worker-pool-sandbox-design.md](superpowers/specs/2026-07-02-worker-pool-sandbox-design.md)).
