...
**A5 LLM-agnostic agent runtime — built.** The run pipeline now executes real work: a domain-owned, **LLM-agnostic** agent loop (`LlmAgentRuntime`) drives all six stages (`PLAN → PROVISION → IMPLEMENT → VERIFY → PR → LEARN`), reaching the model only through a single `LLMAdapter` port. Two adapters ship — **`ClaudeLLMAdapter`** (Anthropic SDK, default) and **`LiteLLMAdapter`** (OpenAI-compatible gateway) — chosen by `naaf_llm_provider`; per-role model selection flows from `AgentDefinition.model_alias`. Tools (read/write/edit/grep/bash) run through a `Workspace` port (`LocalWorkspace`); PROVISION clones the project repo + creates the `agent/<run>` branch; the PR stage captures the opened PR URL into a `RunEvent`; LEARN runs as the curator. Runs execute **locally in the worker** (no sandbox yet). This **supersedes the design's "Claude Code runtime adapter" framing** — we own the loop, so the whole project is provider-agnostic. Deferred: sandbox / egress / GitHub App (A4), budget enforcement (A5d), memory-diff review UI (A6). See [specs/2026-07-02-a5-llm-agnostic-agent-runtime-design.md](specs/2026-07-02-a5-llm-agnostic-agent-runtime-design.md) + the matching plan.
