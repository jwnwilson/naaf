...
> With `naaf_llm_provider=claude` and an API key configured, starting a run drives `PLAN → IMPLEMENT → VERIFY` via a **domain `AgentRuntime`** that calls an LLM through the `LLMAdapter` port, uses a tool set to read/edit files and run commands in a local workspace, streams real agent logs to the Agent-Monitor, and produces a real branch with committed changes and a QA verdict. Switching `naaf_llm_provider=litellm` routes the identical runtime through the LiteLLM gateway with no code change. `make coverage` (80%) + `make lint` green; the loop is fully unit-tested with a `FakeLLMAdapter` (no network).
...
- **PROVISION / PR / LEARN stay stubbed** (as in A3). Real workspace provisioning, sandbox hardening, egress proxy, GitHub App, and real PRs are **A4**; the curator/memory LEARN stage is **A6**. A5 makes the three **LLM-driven** stages (`PLAN`, `IMPLEMENT`, `VERIFY`) real. *(Open decision — see §3; the user earlier favored "full 6 stages". This spec recommends the A3/A4/A6 milestone boundary and flags it for review.)*
...
- Real per-run sandbox isolation. A5 runs the loop **locally in the worker** against a local workspace with the operator's existing git/`gh` credentials.
...
| **Stage scope (OPEN)** | **Recommended:** real `PLAN/IMPLEMENT/VERIFY`; `PROVISION/PR/LEARN` stay stubbed until A4/A6 | Respects milestone boundaries; real PR needs A4's GitHub App, LEARN needs A6's curator. **Confirm or override in review.** |
...
## 6. Stage → agent-loop mapping (real in A5)
...
`PROVISION` / `PR` / `LEARN` remain the A3 stubs pending A4/A6 (see §3 open decision).
...
4. **Worker wiring + `StageContext`** — `_deps()` provider selection + `handlers.py` context population (coordinated with A3); end-to-end local run.
5. **`LiteLLMAdapter`** — gateway translation, alias routing, per-run budget key; compose service + docs.
