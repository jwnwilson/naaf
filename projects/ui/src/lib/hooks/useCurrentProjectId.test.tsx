...
test("prefers the route :projectId over the ?project= search param", async () => {
  server.use(
    http.get("/api/projects", () =>
      HttpResponse.json({ success: true, error: null, data: [{ id: "p1", name: "A", repoUrl: "", itemCount: 0, createdAt: "x", updatedAt: "x" }], meta: { total: 1, page_size: 50, page_number: 1 } }),
    ),
  ...
  function RouteWrapper({ children }: { children: React.ReactNode }) {
    ...
    return (
      <QueryClientProvider client={qc}>
        <MemoryRouter initialEntries={["/projects/route-id/items?project=param-id"]}>
          <Routes>
            <Route path="/projects/:projectId/items" element={children} />
          </Routes>
        </MemoryRouter>
      </QueryClientProvider>
    );
  }
  const { result } = renderHook(() => useCurrentProjectId(), { wrapper: RouteWrapper });
  await waitFor(() => expect(result.current).toBe("route-id"));
});
