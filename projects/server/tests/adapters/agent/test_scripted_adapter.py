...
REPORT_TOOL = ToolSpec(
    name="report", description="", parameters={"type": "object", "properties": {}}
)
CREATE_TOOL = ToolSpec(
    name="create_work_item", description="", parameters={"type": "object", "properties": {}}
)
...
def test_lead_plan_walks_list_epic_feature_task_proposerun_then_text():
    ...
    # step 0 → list_board
    r = next_call()
    assert r.tool_calls[0].name == "list_board"
    msgs += [
        LLMMessage(role=MessageRole.ASSISTANT, tool_calls=r.tool_calls),
        LLMMessage(
    ...
    # step 1 → create epic
    r = next_call()
    assert r.tool_calls[0].args == {"kind": "epic", "title": EPIC_TITLE}
    msgs += [
        LLMMessage(role=MessageRole.ASSISTANT, tool_calls=r.tool_calls),
        LLMMessage(
            role=MessageRole.TOOL,
            content="created epic 'E2E Epic' id=epic123",
            tool_call_id=r.tool_calls[0].id,
        ),
    ]
    # step 2 → create feature under epic123
    r = next_call()
    expected_args = {"kind": "feature", "title": FEATURE_TITLE, "parent_id": "epic123"}
    assert r.tool_calls[0].args == expected_args
    msgs += [
        LLMMessage(role=MessageRole.ASSISTANT, tool_calls=r.tool_calls),
        LLMMessage(
            role=MessageRole.TOOL,
            content="created feature 'E2E Feature' id=feat456",
            tool_call_id=r.tool_calls[0].id,
        ),
    ]
    # step 3 → create task under feat456
    r = next_call()
    expected_task_args = {"kind": "task", "title": TASK_TITLE, "parent_id": "feat456"}
    assert r.tool_calls[0].args == expected_task_args
    msgs += [
        LLMMessage(role=MessageRole.ASSISTANT, tool_calls=r.tool_calls),
        LLMMessage(
            role=MessageRole.TOOL,
            content="created task 'E2E streaming task' id=task789",
            tool_call_id=r.tool_calls[0].id,
        ),
    ]
    # step 4 → propose_run on task789
    r = next_call()
    assert r.tool_calls[0].name == "propose_run"
    assert r.tool_calls[0].args == {"work_item_ids": ["task789"]}
    msgs += [
        LLMMessage(role=MessageRole.ASSISTANT, tool_calls=r.tool_calls),
        LLMMessage(
            role=MessageRole.TOOL,
            content="proposed run",
            tool_call_id=r.tool_calls[0].id,
        ),
    ]
    # step 5 → final text, no tool calls
    r = next_call()
    assert r.stop_reason == "end_turn" and not r.tool_calls and TASK_TITLE in r.content
