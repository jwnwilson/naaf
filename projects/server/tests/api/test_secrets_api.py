...
def _client(session_factory, async_session_factory, key: str | None = None) -> TestClient:
...
def test_set_then_list_masks_the_value(session_factory, async_session_factory):
    c = _client(session_factory, async_session_factory)
...
def test_unset_secret_lists_as_not_set(session_factory, async_session_factory):
    c = _client(session_factory, async_session_factory)
...
def test_unknown_name_is_422(session_factory, async_session_factory):
    c = _client(session_factory, async_session_factory)
    assert c.put("/secrets/aws_key", json={"value": "x"}).status_code == 422


def test_blank_value_is_422(session_factory, async_session_factory):
    c = _client(session_factory, async_session_factory)
    assert c.put("/secrets/github_token", json={"value": "   "}).status_code == 422


def test_delete_clears_the_secret(session_factory, async_session_factory):
    c = _client(session_factory, async_session_factory)
...
def test_missing_encryption_key_fails_closed(session_factory, async_session_factory):
    c = _client(session_factory, async_session_factory, key="")
